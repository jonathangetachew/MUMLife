...
public class ReservationRecordResourceIT {
    ...
    private static final ZonedDateTime DEFAULT_EXPIRATION_DATE = ZonedDateTime.ofInstant(Instant.ofEpochMilli(0L),
            ZoneOffset.UTC);
    ...
    public static ReservationRecord createEntity(EntityManager em) {
        ReservationRecord reservationRecord = new ReservationRecord().expirationDate(DEFAULT_EXPIRATION_DATE)
    ...
    public static ReservationRecord createUpdatedEntity(EntityManager em) {
        ReservationRecord reservationRecord = new ReservationRecord().expirationDate(UPDATED_EXPIRATION_DATE)
    ...
    public void createReservationRecord() throws Exception {
        ...
        restReservationRecordMockMvc
                .perform(post("/api/reservation-records").contentType(TestUtil.APPLICATION_JSON_UTF8)
    ...
    public void getAllReservationRecords() throws Exception {
        ...
        restReservationRecordMockMvc.perform(get("/api/reservation-records?sort=id,desc")).andExpect(status().isOk())
    ...
    public void updateReservationRecord() throws Exception {
        ...
        ReservationRecord updatedReservationRecord = reservationRecordRepository.findById(reservationRecord.getId())
                .get();
        // Disconnect from session so that the updates on updatedReservationRecord are
        // not directly saved in db
        em.detach(updatedReservationRecord);
        updatedReservationRecord.expirationDate(UPDATED_EXPIRATION_DATE).createdAt(UPDATED_CREATED_AT);

        restReservationRecordMockMvc
                .perform(put("/api/reservation-records").contentType(TestUtil.APPLICATION_JSON_UTF8)
