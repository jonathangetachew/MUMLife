...
public class ReservationRecordResourceIT {
    ...
    /**
     * This repository is mocked in the edu.mum.life.repository.search test package.
     *
     * @see edu.mum.life.repository.search.ReservationRecordSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createReservationRecord() throws Exception {
        ...
        // Validate the ReservationRecord in Elasticsearch
        verify(mockReservationRecordSearchRepository, times(1)).save(testReservationRecord);
    }
    ...
    public void createReservationRecordWithExistingId() throws Exception {
        ...
        // Validate the ReservationRecord in Elasticsearch
        verify(mockReservationRecordSearchRepository, times(0)).save(reservationRecord);
    }
    ...
    public void updateReservationRecord() throws Exception {
        ...
        reset(mockReservationRecordSearchRepository);
    ...
    public void deleteReservationRecord() throws Exception {
        ...
        // Validate the ReservationRecord in Elasticsearch
        verify(mockReservationRecordSearchRepository, times(1)).deleteById(reservationRecord.getId());
    }

    @Test
    @Transactional
    public void searchReservationRecord() throws Exception {
        // Initialize the database
        reservationRecordService.save(reservationRecord);
        when(mockReservationRecordSearchRepository.search(queryStringQuery("id:" + reservationRecord.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(reservationRecord), PageRequest.of(0, 1), 1));
        // Search the reservationRecord
        restReservationRecordMockMvc.perform(get("/api/_search/reservation-records?query=id:" + reservationRecord.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(reservationRecord.getId().intValue())))
            .andExpect(jsonPath("$.[*].expirationDate").value(hasItem(sameInstant(DEFAULT_EXPIRATION_DATE))))
