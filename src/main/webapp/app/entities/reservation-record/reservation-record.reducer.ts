...
export const ACTION_TYPES = {
  SEARCH_RESERVATIONRECORDS: 'reservationRecord/SEARCH_RESERVATIONRECORDS',
...
export default (state: ReservationRecordState = initialState, action): ReservationRecordState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_RESERVATIONRECORDS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_RESERVATIONRECORDS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_RESERVATIONRECORDS):
...
const apiSearchUrl = 'api/_search/reservation-records';

// Actions

export const getSearchEntities: ICrudSearchAction<IReservationRecord> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_RESERVATIONRECORDS,
  payload: axios.get<IReservationRecord>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
