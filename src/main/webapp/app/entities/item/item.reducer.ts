...
export const getSearchEntities: ICrudSearchAction<IItem> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_ITEMS,
  payload: axios.get<IItem>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
