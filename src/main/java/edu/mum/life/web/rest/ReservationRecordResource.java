...
public class ReservationRecordResource {
    ...
    /**
     * {@code SEARCH  /_search/reservation-records?query=:query} : search for the reservationRecord corresponding
     * to the query.
     *
     * @param query the query of the reservationRecord search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/reservation-records")
    public ResponseEntity<List<ReservationRecord>> searchReservationRecords(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ReservationRecords for query {}", query);
        Page<ReservationRecord> page = reservationRecordService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
